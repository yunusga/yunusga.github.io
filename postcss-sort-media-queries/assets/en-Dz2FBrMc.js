...
`,this.listeners(),this.runSorter()}listeners(){this.$source.addEventListener(`keyup`,this.runSorter.bind(this),!1),this.$methods.forEach(e=>{e.addEventListener(`change`,this.runSorter.bind(this),!1)}),this.$unitlessMqAlwaysFirst.addEventListener(`click`,this.runSorter.bind(this),!1)}runSorter(){let e=this.$source.value,t={sort:this.getSortingMethod(),configuration:{unitlessMqAlwaysFirst:this.$unitlessMqAlwaysFirst.checked}};this.postcss([this.mqSorter(t)]).process(e).then(e=>{let t=``;this.prettier.format(e.css,{parser:`css`,plugins:[this.parserCss]}).then(e=>{t+=this.textPrepare(e),this.$sorted.innerHTML=t,(0,l.highlightElement)(this.$sorted)})}).catch(e=>{console.log(e),this.$sorted.innerHTML=this.textPrepare(e.toString())})}textPrepare(e=``){return e.replace(/\</g,`&lt;`).replace(/\>/g,`&gt;`)}getSortingMethod(){return console.log(this.$methods),[...this.$methods].filter(e=>e.checked)[0].value}};export{c as n,o as t};